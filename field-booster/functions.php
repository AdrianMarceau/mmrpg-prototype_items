...
$functions = array(
    ...
    'rpg-robot_check-items_end-of-turn' => function($objects){
        ...
        // Create a list of types to boost given this robot's core(s)
        $field_booster_types = array();
        if (empty($this_robot->robot_core)){ $field_booster_types[] = 'recovery';  }
        elseif ($this_robot->robot_core === 'empty'){ $field_booster_types[] = 'damage'; }
        elseif ($this_robot->robot_core === 'copy'){ $field_booster_types[] = !empty($this_field->field_type) ? $this_field->field_type : 'experience'; }
        else { $field_booster_types[] = $this_robot->robot_core; }
        if (!empty($this_robot->robot_core2)){
            if ($this_robot->robot_core2 === 'empty'){ $field_booster_types[] = 'damage'; }
            elseif ($this_robot->robot_core2 === 'copy'){ $field_booster_types[] = !empty($this_field->field_type) ? $this_field->field_type : 'experience'; }
            else { $field_booster_types[] = $this_robot->robot_core2; }
        }

        // Loop through the list of types to boost and do so, one at a time
        foreach ($field_booster_types AS $temp_boost_key => $temp_boost_type){
